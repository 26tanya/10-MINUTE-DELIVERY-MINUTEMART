...
// Helper to load all products from multiple subcategories
async function loadMultipleSubcategoryProducts(pincode, subcategories) {
  const productMain = document.querySelector("main");
  productMain.innerHTML = ""; // Clear main content

  // const fetches = subcategories.map(subcat =>
  //   fetch(`product.html?pincode=${pincode}&subcategory_id=${subcat.Subcategory_ID}`)
  //     .then(res => res.text())
  //     .then(html => {
  //       const parser = new DOMParser();
  //       const doc = parser.parseFromString(html, 'text/html');
  //       const productMainInner = doc.querySelector("main");
  //       return productMainInner ? productMainInner.innerHTML : "";
  //     })
  //     .catch(err => {
  //       console.error("❌ Failed loading subcategory products:", err);
  //       return "";
  //     })
  // );

  const fetches = subcategories.map(subcat => {
    console.log("Fetching products for subcategory:", subcat.Subcategory_ID, "Pincode:", pincode); // Add logging here
    return fetch(`product.html?pincode=${pincode}&subcategory_id=${subcat.Subcategory_ID}`)
      .then(res => res.text())
      .then(html => {
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, 'text/html');
  ...
  try {
    ...
    // Load product.js only once
    if (!document.querySelector('script[src="product.js"]')) {
      const script = document.createElement("script");
      script.src = "product.js";
      script.onload = () => {
        if (typeof initProductPage === "function") {
      ...
      // product.js already loaded, just re-init
      if (typeof initProductPage === "function") {
        initProductPage();
      }
    }
